"""评估“先扫完覆盖、x 范围内顺路清除、最后按最短路清剩余”的调度，并扫描关键参数。
复用同目录 调度改进原型.py（其又复用 code/baseline/最终方案验证.py 的几何函数与 World 仿真）。本地仿真，非官方成绩。
参数：
...
运行：python 参数扫描_顺路清除x.py   （约 1–2 分钟）
"""
import math, copy, random, statistics as st, importlib.util
from pathlib import Path
_p=Path(__file__).resolve().parent/'调度改进原型.py'
...
if __name__=='__main__':
    ...
    print('交接版每源平均：问题三 %.0f s，问题四 %.0f s'%(base3,base4))

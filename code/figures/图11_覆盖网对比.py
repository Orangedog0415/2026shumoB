...
运行：python 图11_覆盖网对比.py   需要 numpy、scipy、matplotlib
...
def draw_net(ax, pts, mode):
    ...
    S.tag(ax, mid, '最长边 %.1f m'%longest[0], dx=26*nx/nl, dy=26*ny/nl)
    return longest[0]
...
if __name__ == '__main__':
    S.use_style()
    NETS = [
        ('(a) 问题三·正八边形覆盖网', L.NET3(), 'disk',
         '证书：距离覆盖\n任一点到最近覆盖点 %.1f m < 1000 m', 'lower left'),
        ('(b) 问题四·方案四 规则三角格 h=950 m', L.NET4_V4(), 'tri',
         '证书：三角网　相交三角形三边 ≤1000 m', 'lower left'),
        ('(c) 问题四·方案七 非规则三角网', L.NET4_V7(), 'tri',
         '证书：三角网（同上）\n最大方位间隔 179.86° < 180°', 'lower right'),
    ]
    ...
    for ax, (title, net, mode, cert, nloc) in zip(axes, NETS):
        ...
        if mode == 'disk':
            ...
            S.tag(ax, wp, '最坏覆盖点', dy=22)
        out = sum(1 for p in net if math.hypot(*p) > R)
        head = '%d 个覆盖点（%d 点在目标圆外）　巡回 %s m' % (len(net), out, format(round(ln), ',')) \
               if out else '%d 个覆盖点　巡回 %s m' % (len(net), format(round(ln), ','))
        S.note(ax, '%s\n%s' % (head, cert), loc=nloc)
        ax.set_xlabel('x / m')
    axes[0].set_ylabel('y / m')
    h, lb = axes[0].get_legend_handles_labels()
    fig.legend(h, lb, loc='lower center', ncol=2, bbox_to_anchor=(.5,-.035),
